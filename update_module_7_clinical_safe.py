...
def run_backend():
    print("--- MOTOR MODULO 7: EXTRAINDO DADOS CLINICOS ---")
    ...
    col_label = next((c for c in all_cols if 'comercial' in str(c).lower()), None)
    col_content = next((c for c in all_cols if 'apresenta' in str(c).lower()), None)
...
def run_frontend():
    # Print sem emoji para evitar erro no Windows
    print("--- ATUALIZANDO FRONTEND (COLUNA CLINICA) ---")
...
<head>
    ...
    <style>
        ...
        .clin-alert { color: #b91c1c; background: #fef2f2; border-color: #fecaca; }
            ...
            <table class="w-full text-sm text-left">
                <thead class="bg-slate-50 uppercase text-xs font-bold text-slate-500 border-b">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-center text-pink-600 w-12">Clín</th>
