...
# HTML COMPLETO COM TODAS AS COLUNAS TRAVADAS
html_code = """<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <title>OncoCalc Pro - Módulo 1 Completo</title>
    ...
    <style>
        ...
        /* Estilos de Status */
        .safe { color: #059669; font-weight: 600; background: #ecfdf5; padding: 2px 6px; rounded: 4px; border: 1px solid #a7f3d0; border-radius: 4px; font-size: 0.75rem; }
        .alert { color: #dc2626; font-weight: 600; background: #fef2f2; padding: 2px 6px; rounded: 4px; border: 1px solid #fecaca; border-radius: 4px; font-size: 0.75rem; }
...
<body onclick="document.getElementById('context-menu').style.display='none'">
    ...
    <div class="container mx-auto p-4 max-w-full">
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 font-bold uppercase">Módulo 1: Identificação & Dados Básicos</p>
                </div>
            </div>
        ...
        <div class="relative mb-6">
            <input type="text" id="search" class="w-full p-3 pl-10 border border-slate-200 rounded-lg shadow-sm outline-none focus:ring-2 focus:ring-blue-500" placeholder="Buscar medicamento...">
    ...
    const App = {
        ...
        render: (list) => {
            ...
            Object.keys(cats).sort().forEach(cat => {
                ...
                cats[cat].sort((a,b) => a.name.localeCompare(b.name)).forEach(m => {
                    ...
                    // --- 2. APRESENTAÇÃO ---
                    // Tenta ler do banco v4.8 (concentration) ou do Motor 1 (concentration_display)
                    let displayApres = m.concentration || m.concentration_display || '-';
                    // Tratamento visual para múltiplas (pipes)
                    ...
                    let icon = isMulti ? '<span class="cursor-pointer ml-1 text-blue-500 text-xs">📚</span>' : '';
                    let rowClass = "hover:bg-slate-50 transition";
                    let ctxEvent = isMulti ? `oncontextmenu="App.showCtx(event, ${m.id}); return false;"` : '';
                    if (isMulti) rowClass += " cursor-context-menu";

                    // --- 4. DILUENTES ---
                    // Normaliza Sim/Não/Nao/-
                    ...
                    // RENDERIZAÇÃO DA LINHA (7 COLUNAS)
                    tb.innerHTML += `
                    <tr class="${rowClass}" ${ctxEvent}>
                        <td class="px-4 py-3 align-top">${brand}</td>
                        <td class="px-4 py-3 align-top font-bold text-slate-700">${m.name} ${icon}</td>
                        <td class="px-4 py-3 align-top text-slate-600 text-xs leading-snug">${displayApres}</td>
        ...
        showCtx: (e, id) => {
            ...
            // Se tiver JSON estruturado (Motor 1) usa ele, senão tenta improvisar ou não abre
            let content = '';
            try {
                const p = JSON.parse(m.presentations);
                if(p.length) {
                    content = p.map(pr => `<div class="ctx-item"><span class="font-bold text-slate-700">${pr.brand}</span> <span class="text-xs text-slate-500">${pr.description}</span></div>`).join('');
                }
            } catch(err) {
                // Se não tiver JSON (veio do v4.8 texto plano), não mostra menu complexo, apenas aviso
                // content = '<div class="p-3 text-xs text-gray-500">Detalhes visíveis na coluna.</div>';
            }
...
print("[SUCESSO] HTML restaurado com as 7 colunas TRAVADAS.")
