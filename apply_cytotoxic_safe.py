...
# LISTA DE RADICAIS CITOTÓXICOS (NIOSH/Oncologia)
cytotoxic_keywords = [
    # Classes Principais
...
def run_backend():
    print("--- CLASSIFICANDO CITOTOXICOS (BACKEND) ---")
    ...
    for r in rows:
        ...
        if is_cyto:
            # Print seguro sem emoji
            print(f" [ALERTA] {name}")
            count += 1
            
    conn.commit()
    conn.close()
    print(f"--- SUCESSO: {count} medicamentos marcados como perigosos. ---")

def run_frontend():
    print("--- ATUALIZANDO HTML COM DESTAQUE VISUAL ---")
...
<head>
    ...
    <style>
        ...
        /* ESTILO CITOTÓXICO (Alerta Vermelho) */
        ...
        render: (list) => {
            ...
            Object.keys(cats).sort().forEach(cat => {
                ...
                cats[cat].sort((a,b) => a.name.localeCompare(b.name)).forEach(m => {
                    ...
                    // LÓGICA VISUAL DE RISCO
                    let rowClass = "hover:bg-slate-50 relative border-b border-slate-50";
                    let cytoBadge = "";
                    
                    if (m.is_cytotoxic === 1) {
                        // Fundo avermelhado e borda lateral
                        rowClass = "row-cytotoxic relative border-b border-red-100";
                        cytoBadge = '<span class="cyto-badge" title="Medicamento Citotóxico / Alto Risco">☣️</span>';
                    }
