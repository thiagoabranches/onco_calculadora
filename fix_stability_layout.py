...
<body onclick="App.closeAll()" class="h-screen flex flex-col overflow-hidden">
    ...
    <div id="calc-modal" class="modal-overlay" onclick="Calc.close(event)">
        <div class="modal-content" style="max-width: 550px;">
            <div class="modal-header bg-indigo-50">
                <div class="flex items-center gap-2"><span class="text-2xl">🧮</span><h3 class="text-lg font-bold text-indigo-900">Calculadoras Clínicas</h3></div>
            ...
            <div class="px-6 border-b border-slate-100 flex gap-4 bg-white">
                <div class="tab-btn active" onclick="Calc.tab('calvert', this)">Calvert</div>
                <div class="tab-btn" onclick="Calc.tab('bsa', this)">SC (BSA)</div>
                <div class="tab-btn" onclick="Calc.tab('crcl', this)">ClCr</div>
            </div>
            <div class="modal-body">
                <div id="tab-calvert" class="tab-content active">
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div><label class="calc-label">Sexo</label><select id="cal-sex" class="calc-input bg-white"><option value="M">Masculino</option><option value="F">Feminino</option></select></div>
                        <div><label class="calc-label">Idade (anos)</label><input type="number" id="cal-age" class="calc-input"></div>
                        <div><label class="calc-label">Peso (kg)</label><input type="number" id="cal-weight" class="calc-input"></div>
                        <div><label class="calc-label">Altura (cm)</label><input type="number" id="cal-height" class="calc-input"></div>
                        <div><label class="calc-label">Creatinina (mg/dL)</label><input type="number" id="cal-scr" class="calc-input" step="0.01"></div>
                        <div><label class="calc-label text-indigo-600">AUC Alvo</label><input type="number" id="cal-auc" class="calc-input border-indigo-200 bg-indigo-50"></div>
                    </div>
                    <div class="flex items-center gap-3 mb-6 bg-slate-50 p-3 rounded-lg border border-slate-200">
                        ...
                        <label for="cal-cap" class="text-xs font-bold text-slate-600 cursor-pointer select-none">Cap Conservador (Max 100 mL/min)</label>
                    </div>
                    <button onclick="Calc.runCalvertAdvanced()" class="w-full bg-indigo-600 text-white py-3 rounded-lg font-bold hover:bg-indigo-700 shadow-md">Calcular</button>
                    <div id="res-cal" class="calc-result hidden text-left bg-white border border-indigo-100 shadow-sm">
                        <div class="text-center border-b border-slate-100 pb-3 mb-3"><span class="text-xs font-bold text-slate-400 uppercase tracking-widest">Dose Final</span><br><span class="text-4xl font-black text-indigo-600" id="val-dose-final">000 mg</span></div>
                        <div class="grid grid-cols-2 gap-2 text-xs text-slate-500">
                            <div class="bg-slate-50 p-2 rounded"><span class="block text-[0.65rem] uppercase font-bold text-slate-400">TFG Calc.</span><span id="val-gfr-raw" class="font-mono font-bold text-slate-700 text-lg">00</span> mL/min</div>
                            <div class="bg-slate-50 p-2 rounded"><span class="block text-[0.65rem] uppercase font-bold text-slate-400">TFG Usada</span><span id="val-gfr-used" class="font-mono font-bold text-slate-700 text-lg">00</span> mL/min</div>
                        </div>
                        <div class="mt-2 text-xs text-slate-400">Peso: <span id="val-weight-type" class="font-bold text-slate-600">Real</span></div>
    ...
    const Calc = {
        ...
        runCalvertAdvanced: () => {
            ...
            if (gfrRaw < 20) {
                warnDiv.innerText = "ALERTA: TFG < 20 mL/min. Calvert não recomendado. Risco de toxicidade.";
                ...
                warnDiv.innerText = "Nota: TFG limitada a 100 mL/min (Conservador).";
                warnDiv.classList.remove('hidden');
        ...
        render: (list) => {
            ...
            Object.keys(cats).sort().forEach(cat => {
                ...
                cats[cat].sort((a,b) => a.name.localeCompare(b.name)).forEach(m => {
                    ...
                    // ÍCONES COM ESCAPE SEGURO
                    // 1. Estabilidade (CORREÇÃO DE ASPAS)
                    let stabRaw = `Rec: ${m.stability_reconst}<br>Dil: ${m.stability_diluted}`;
                    // Limpa caracteres perigosos para HTML
