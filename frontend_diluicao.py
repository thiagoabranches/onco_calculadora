...
<head>
    ...
    <style>
        ...
        /* PADRONIZAÇÃO (NOVOS ESTILOS) */
        .std-badge { display: inline-block; padding: 6px 12px; border-radius: 6px; font-weight: 700; font-size: 0.8rem; text-align: center; min-width: 100px; box-shadow: 0 1px 2px rgba(0,0,0,0.05); }
...
<body onclick="App.closeAll()" class="h-screen flex flex-col overflow-hidden">
    ...
    <header class="bg-white border-b border-slate-200 shadow-sm z-20 flex-shrink-0 h-16 flex items-center justify-between px-6">
        ...
        <div class="flex gap-4 items-center">
            <button onclick="Calc.open()" class="flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 font-semibold text-sm transition border border-indigo-100"><span>🧮</span> Calculadoras</button>
            ...
            <button onclick="window.location.reload()" class="p-2 text-slate-400 hover:text-blue-600 hover:bg-slate-50 rounded-lg transition" title="Atualizar"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg></button>
        </div>
    </header>
        ...
        render: (list) => {
            ...
            Object.keys(cats).sort().forEach(cat => {
                ...
                cats[cat].sort((a,b) => a.name.localeCompare(b.name)).forEach(m => {
                    ...
                    // LÓGICA PADRONIZAÇÃO (COR RIGOROSA)
