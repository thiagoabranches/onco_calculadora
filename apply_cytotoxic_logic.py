...
# LISTA DE PALAVRAS-CHAVE PARA IDENTIFICAR CITOTÓXICOS
# Baseado em radicais farmacológicos e listas de antineoplásicos perigosos (NIOSH)
cytotoxic_keywords = [
    # Classes Principais
    "rubicina", "platina", "taxel", "posideo", "posídeo", "tecan", 
    "vinca", "vinbla", "vincri", "vinore", "vinflu", 
    "fosphamide", "fosfamida", "melfalan", "bussulfan", "tiotepa", 
    "mustina", "mustine", "bendamustina", "dacarbazina", "temozolomida", 
    "trabectedin", "lurbinectedin", "eribulin", "ixabepilon",
    
    # Antimetabólitos
    "fluorouracil", "capecitabina", "citarabina", "gencitabina", 
    "pemetrexede", "metotrexato", "fludarabina", "cladribina", 
    "azacitidina", "decitabina", "nelarabina", "clofarabina",
    
    # Antibióticos Antitumorais
    "mitomicina", "bleomicina", "dactinomicina", "mitoxantrona",
    
    # Inibidores de Proteassoma (Alto Risco)
    "bortezomib", "carfilzomib", "ixazomib",
    
    # ADCs (Anticorpos Conjugados a Drogas - Carga Citotóxica)
    "vedotin", "govitecan", "tansina", "deruxtecan", "ozogamicina"
]

# Exceções (Se precisar)
safe_keywords = [
    # Adicione aqui se algum match acima for falso positivo
]

def is_cytotoxic(name):
    n = name.lower()
    for kw in cytotoxic_keywords:
        if kw in n:
            return 1 # True
    return 0 # False

def run_backend():
    print("--- CLASSIFICANDO CITOTÓXICOS ---")
    ...
    for r in rows:
        ...
        is_cyto = is_cytotoxic(name)
        
        cursor.execute("UPDATE medicamentos SET is_cytotoxic = ? WHERE id = ?", (is_cyto, mid))
        if is_cyto:
            print(f" [☣️] {name}")
            count += 1
    ...
    conn.commit()
    conn.close()
    print(f"--- SUCESSO: {count} medicamentos marcados como citotóxicos. ---")

def run_frontend():
    print("--- ATUALIZANDO HTML (DESTAQUE VERMELHO) ---")
...
<head>
    ...
    <style>
        ...
        /* Destaque Citotóxico */
        .row-cytotoxic { background-color: #fef2f2 !important; border-left: 4px solid #ef4444; }
        .row-cytotoxic:hover { background-color: #fee2e2 !important; }
        .cyto-badge { color: #dc2626; font-size: 0.8em; margin-left: 4px; vertical-align: middle; }
...
<body onclick="App.closeAll()">
            ...
            <table class="w-full text-sm text-left">
                <thead class="bg-slate-50 uppercase text-xs font-bold text-slate-500 border-b">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-center w-24">Farmacotécnica</th>
                        <th class="px-4 py-3 text-center text-blue-600 w-24">Fís-Quím</th>
    ...
    const App = {
        ...
        quickPhys: (e, id) => {
            e.preventDefault(); e.stopPropagation();
            const m = App.data.find(x => x.id === id);
            App.showCtx(e, `${m.name} (Segurança)`, [['Caráter', m.phys_carater], ['Filtro', m.phys_filtro], ['Fotossensível', m.phys_foto]]);
        },
        ctxBrands: (e, brands) => {
            e.preventDefault(); e.stopPropagation();
