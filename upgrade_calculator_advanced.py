...
<body onclick="App.closeAll()" class="h-screen flex flex-col overflow-hidden">
        ...
        <div class="modal-content" style="max-width: 550px;">
            <div class="modal-header bg-indigo-50">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-lg font-bold text-indigo-900">Calculadoras Clínicas</h3>
                </div>
            ...
            <div class="px-6 border-b border-slate-100 flex gap-4 bg-white">
                <div class="tab-btn active" onclick="Calc.tab('calvert', this)">Calvert (Carboplatina)</div>
                <div class="tab-btn" onclick="Calc.tab('bsa', this)">SC (BSA)</div>
                <div class="tab-btn" onclick="Calc.tab('crcl', this)">ClCr (Rim)</div>
            </div>
            <div class="modal-body">
                
                <div id="tab-calvert" class="tab-content active">
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            <label class="calc-label">Sexo</label>
                            <select id="cal-sex" class="calc-input bg-white"><option value="M">Masculino</option><option value="F">Feminino</option></select>
                        </div>
                        <div>
                            <label class="calc-label">Idade (anos)</label>
                            <input type="number" id="cal-age" class="calc-input" placeholder="Ex: 65">
                        </div>
                        <div>
                            <label class="calc-label">Peso (kg)</label>
                            <input type="number" id="cal-weight" class="calc-input" placeholder="Ex: 70">
                        </div>
                        <div>
                            <label class="calc-label">Altura (cm)</label>
                            <input type="number" id="cal-height" class="calc-input" placeholder="Ex: 175">
                        </div>
                        <div>
                            <label class="calc-label">Creatinina (mg/dL)</label>
                            <input type="number" id="cal-scr" class="calc-input" step="0.01" placeholder="Ex: 0.9">
                        </div>
                        <div>
                            <label class="calc-label text-indigo-600">AUC Alvo</label>
                            <input type="number" id="cal-auc" class="calc-input border-indigo-200 bg-indigo-50" placeholder="Ex: 5 ou 6">
                        </div>
                    </div>
                    
                    <div class="flex items-center gap-3 mb-6 bg-slate-50 p-3 rounded-lg border border-slate-200">
                        <input type="checkbox" id="cal-cap" class="w-5 h-5 text-blue-600 rounded border-gray-300 focus:ring-blue-500">
                        <label for="cal-cap" class="text-xs font-bold text-slate-600 cursor-pointer select-none">
                            Cap Conservador (Limitar TFG a 100 mL/min)
                        </label>
                    </div>

                    <button onclick="Calc.runCalvertAdvanced()" class="w-full bg-indigo-600 text-white py-3 rounded-lg font-bold hover:bg-indigo-700 shadow-md transition transform active:scale-95">
                        Calcular Dose Carboplatina
                    </button>
                    
                    <div id="res-cal" class="calc-result hidden text-left bg-white border border-indigo-100 shadow-sm">
                        <div class="text-center border-b border-slate-100 pb-3 mb-3">
                            <span class="text-xs font-bold text-slate-400 uppercase tracking-widest">Dose Final (Arredondada)</span><br>
                            <span class="text-4xl font-black text-indigo-600" id="val-dose-final">000 mg</span>
                        </div>
                        <div class="grid grid-cols-2 gap-2 text-xs text-slate-500">
                            <div class="bg-slate-50 p-2 rounded">
                                <span class="block text-[0.65rem] uppercase font-bold text-slate-400">TFG Calculada</span>
                                <span id="val-gfr-raw" class="font-mono font-bold text-slate-700 text-lg">00</span> mL/min
                            </div>
                            <div class="bg-slate-50 p-2 rounded">
                                <span class="block text-[0.65rem] uppercase font-bold text-slate-400">TFG Utilizada</span>
                                <span id="val-gfr-used" class="font-mono font-bold text-slate-700 text-lg">00</span> mL/min
                            </div>
                        </div>
                        <div class="mt-2 text-xs text-slate-400">
                            Peso Usado: <span id="val-weight-type" class="font-bold text-slate-600">Real</span>
                        </div>
                        <div id="cal-warning" class="mt-3 text-xs text-red-700 font-bold bg-red-50 p-3 rounded border border-red-100 hidden"></div>
                        <div class="mt-3 text-[0.65rem] text-slate-300 text-center italic border-t pt-2">
                            Uso estritamente educacional. Valide com médico oncologista.
                        </div>
                    </div>
                </div>

                <div id="tab-bsa" class="tab-content">
                    <label class="calc-label">Peso (kg)</label><input type="number" id="bsa-peso" class="calc-input">
                    <label class="calc-label">Altura (cm)</label><input type="number" id="bsa-altura" class="calc-input">
                    <button onclick="Calc.runBSA()" class="w-full bg-blue-600 text-white py-3 rounded-lg font-bold hover:bg-blue-700 mt-4">Calcular SC</button>
                ...
                <div id="tab-crcl" class="tab-content">
                    <div class="grid grid-cols-2 gap-4">
                        <div><label class="calc-label">Creatinina (mg/dL)</label><input type="number" id="cr-creat" class="calc-input" step="0.1"></div>
                        <div><label class="calc-label">Peso (kg)</label><input type="number" id="cr-peso" class="calc-input"></div>
                        <div><label class="calc-label">Idade (anos)</label><input type="number" id="cr-idade" class="calc-input"></div>
                        <div><label class="calc-label">Sexo</label><select id="cr-sexo" class="calc-input"><option value="M">Masculino</option><option value="F">Feminino</option></select></div>
                    </div>
                    <button onclick="Calc.runCrCl()" class="w-full bg-blue-600 text-white py-3 rounded-lg font-bold hover:bg-blue-700 mt-4">Calcular ClCr</button>
    ...
    <script>
    // LÓGICA CALCULADORA AVANÇADA (CALVERT V2)
    const Calc = {
        ...
        runCalvertAdvanced: () => {
            ...
            if(!age || !weight || !height || !scr || !auc) {
                alert("Preencha todos os campos para cálculo preciso.");
                return;
            }

            // 2. Ajuste de Peso (Obesidade)
            // IBW (Devine)
            let ibw = 0;
            if(sex === 'M') ibw = 50 + 2.3 * ((height/2.54) - 60);
            ...
            let weightLabel = "Real (IMC < 25)";
            
            // Regra: Se IMC >= 25, usar Peso Ajustado
            if (bmi >= 25) {
                weightUsed = ibw + 0.4 * (weight - ibw);
                weightLabel = "Ajustado (IMC ≥ 25)";
            }

            // 3. Ajuste de Creatinina (Baixa Musculatura)
            // Regra: Se Scr < 0.7, usa 0.7 para evitar superestimar TFG
            if (scr < 0.7) {
                scr = 0.7; // Ajuste forçado
                // Poderíamos adicionar um aviso visual aqui se quisesse
            }
            ...
            const gfrRaw = gfr; // Guarda o valor bruto

            // 5. Cap Conservador (100 mL/min) vs Standard (125 mL/min implicit cap)
            let gfrFinal = gfr;
            if (useCap && gfr > 100) {
                gfrFinal = 100;
            } else if (gfr > 125 && !useCap) {
                // Mesmo sem cap conservador, muitas práticas limitam a 125 (FDA)
                // Vamos deixar livre se o check não estiver marcado, ou avisar?
                // O prompt pediu cap conservador de 100.
            }

            // 6. Fórmula Calvert
            const dose = auc * (gfrFinal + 25);
            const doseRounded = Math.round(dose / 10) * 10; // Arredonda para dezena mais próxima (ex: 443 -> 440)
            ...
            if (gfrRaw < 20) {
                warnDiv.innerText = "ALERTA CRÍTICO: TFG < 20 mL/min. A fórmula de Calvert não é recomendada devido à baixa precisão e alto risco de toxicidade. Consulte o oncologista.";
                warnDiv.classList.remove('hidden');
            } else if (useCap && gfrRaw > 100) {
                warnDiv.innerText = "Nota: TFG foi limitada a 100 mL/min conforme opção conservadora.";
                warnDiv.classList.remove('hidden');
                warnDiv.classList.replace('text-red-700', 'text-blue-700');
                warnDiv.classList.replace('bg-red-50', 'bg-blue-50');
                warnDiv.classList.replace('border-red-100', 'border-blue-100');
            }
